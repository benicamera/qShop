...
def learn_and_predict(list_objects):
    ...
    print("P out", prediction_output)
    print("Liste", item_list_output)
    ...
    """for i, o in enumerate(item_list_output):       # Nur Items mit einer Kaufwahrscheinlichkeit von
        ...
        if prediction_output[i] > 0.6:
            output.insert(0, o)"""
    for i in range(-1, -1 * len(item_list_output), -1):
        output.append(item_list_output[i])
        if i == -12:
            break
...
if __name__ == '__main__':
    objekte = ["apfel", "karotte", "birne", "tomate", "mango", "trauben", "salat", "brot", "toast", "marmelade", "schokolade", "kekse", "nudeln", "reis"]
